...
NHdata_keep$Residency <- ifelse(NHdata_keep$out_of_state,"Out of State","In State")

#create the barchart: Residency vs Stop Outcome
getOption("scipen")
opt <- options("scipen" = 20)
x <- table(NHdata_keep$stop_outcome, NHdata_keep$Residency)
barplot(x, xlab = "Residency", ylab = "Stop Outcome", main = "Residency vs Stop Outcome",
        col = c("Orange", "Grey", "Pink", "LightBlue"), beside = TRUE)

#include the legend in the chart
legend("topright", legend=c("Checkup", "Summons", "Ticket", "Warning"), col=c("Orange", "Grey", "Pink", "LightBlue"), lty=c(1),lwd=c(4),pch=c(NA))

# Name: Yi-Ming Chang (Amy)

#check and setup the working directoy
getwd()
setwd("/Users/amy/Desktop/Amy/UNH/Classes/Data 801/R Programming/WK3 - 20180612")
getwd()
NHdata <- read.csv("NH_cleaned.csv")
